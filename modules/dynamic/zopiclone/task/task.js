...
var notes_pid=m_var.clinical_trials_notes_tid;
var site_filter_pid=m_var.site_filter_tid;
...
_set_req=function(){
    ...
    sql+=",participant as (select Site=S1,ParticipantUID=UID,sql_participant="+_module.var.sql_participant+" from [FORM-"+participant_pid+"]"+site_sql_where+participant_where+" )";
    sql+=",task as (select ID,UID,PUID,S3,Site=participant.Site,Information,ParticipantUID,sql_participant,DateTime,Author,RowNum=row_number() over (order by ID DESC) from [FORM-"+_db_pid+"-@S1] join participant on PUID=ParticipantUID)";
    sql+="select ID,S3,UID,Site,Information,ParticipantUID,Participant=sql_participant,DateTime,Author,RowNum,NT,NC,dirty=0, valid=1 from task left join notes on UID=notes.PUID and NRowNum=1 where RowNum between @I6 and @I7";
...
_set_req_export=function(){
    _fields_e="Participant_ID|ParticipantUID,Participant,"+_task_fields
    ...
    sql+=" select ID,Site,Information,ParticipantUID,Participant=sql_participant,DateTime,Author from participant left join task on PUID=ParticipantUID";
    _set_from_to();
    ...
    switch(field){
        ...
        case 'NT':
            ...
            td.find('u').on('click',function(){
                ...
                $vm.load_module_by_name(_app_id+'clinical_trials_notes',$vm.root_layout_content_slot,{
